...
def password_not_recogonised():
    global screen4
    screen4 = Toplevel(screen)
    screen4.title("Success")
    screen4.geometry("150x100")
    Label(screen4, text = "Password not recogonised").pack()
...
def user_not_found():
    ...
    Label(screen5, text="Username not recogonised").pack()
...
def login_verify():
    ...
    list_of_files = os.listdr()
    if username1 in list_of_files:
        verify = file1.read().splitlines()
        if password1 in verify:
            login_success()
        else:
            password_not_recogonised()
...
def login():
    ...
    Label(screen2, text = "Please enter login credentials: ").pack()
